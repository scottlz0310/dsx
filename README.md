...
```bash
# 例: Linux amd64（v0.3.0 の場合）
curl -Lo dsx.tar.gz https://github.com/scottlz0310/dsx/releases/download/v0.3.0/dsx_0.3.0_linux_amd64.tar.gz
tar xzf dsx.tar.gz
...
本バージョンは **Go パッケージ管理機能の新規追加** を含む機能強化リリースです。

### 主な変更

- `GoBinaryInfo` 構造体・`ParseGoBinaryInfo` による Go バイナリ情報の解析基盤を追加
- `dsx sys discover` コマンドで `$GOBIN`/`$GOPATH/bin`/`~/go/bin` 内の Go ツールを自動検出
- Go updater で `targets` 未設定時に `dsx sys discover` への誘導メッセージを表示
