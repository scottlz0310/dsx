...
```bash
# 例: Linux amd64（v0.5.0 の場合）
curl -Lo dsx.tar.gz https://github.com/scottlz0310/dsx/releases/download/v0.5.0/dsx_0.5.0_linux_amd64.tar.gz
tar xzf dsx.tar.gz
...
本バージョンは **Go updater の最新版判定機能追加** を含む機能強化リリースです。

### 主な変更

- Go updater で `go version -m` と `go list -m -json <module>@latest` を比較し、最新版の Go ツールは `go install` をスキップするよう改善
- `dsx self-update` の更新判定ロジックを `internal/selfupdate` に分離し、Go updater と共通利用できるよう整理
- `go.targets` に `github.com/scottlz0310/dsx/cmd/dsx` が含まれる場合、Go updater では dsx 本体を更新せず `dsx self-update` を案内

### 推奨運用（当面）

- `dsx sys discover` で Go バイナリを検出し、`go.targets` に追加してから `dsx sys update` を実行する
