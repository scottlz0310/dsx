...
- [ ] **設定パッケージ (`internal/config`) の作成**
    - [ ] 設定構造体 (`Config`, `RepoConfig`, `SysConfig`) の定義
    - [ ] `viper` を使用したデフォルト値設定とファイル読み込み処理
    - [ ] 環境変数 (`DEVSYNC_*`) とのバインディング
- [ ] **設定ファイル生成 (`config init`)**
    - [ ] `survey` を導入し、対話形式でユーザー入力を受け付ける
    - [ ] 入力内容を `yaml` ファイルとして保存する
- [ ] **環境認識**
    - [ ] コンテナ内 (`IsContainer`) かホストか判定するロジックの実装
