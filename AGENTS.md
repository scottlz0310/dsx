...
- レビュー対応中はサブ PR を新規作成しない。
- Copilot レビューは前回指摘をすべて対応後、1 回のみ依頼する。
...
- 一定時間（2分×3回）新規コメントが発生していない（no comment）かつ CI が成功している
...
- **blocking**（修正必須）:
  - 実行時エラーの可能性がある
  - データ整合性を破壊する可能性がある
  - セキュリティリスク（認証・認可・入力検証）がある
  - 型安全性が欠如している
  - 後方互換性のない変更

- **non-blocking**（任意）:
  - テスト追加・ログ改善など、対応推奨だが必須ではないもの
...
- `non-blocking / suggestion` は必ず以下のいずれかを選択する:
  - 今回対応する
  - 対応しない（理由を明示）
...
- blocking が残存 → 再レビュー依頼（`mcp__copilot-review-mcp__request_copilot_review`）
- blocking がすべて解消済み かつ 再レビュー必須 → 再レビュー依頼
- マージ条件をすべて満たす → マージ

再レビュー依頼前チェック:

- 前回の指摘がすべて resolve されている
- 新たな blocking を発生させていない

### コメント返信・resolve 手順

1. `mcp__copilot-review-mcp__get_review_threads` でスレッド一覧を取得
2. `is_resolved: false` のスレッドをすべて対象にする
3. `mcp__copilot-review-mcp__reply_and_resolve_review_thread` で採否理由を日本語で明示した上で返信・resolve
4. 再レビューが必要な場合は `mcp__copilot-review-mcp__request_copilot_review` を呼び出す

## 7. テスト方針：カバレッジより性質を重視
